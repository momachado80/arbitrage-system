/**
 * Tiny in-house assertion harness for the readiness layer.
 *
 * No external test framework is wired in this repo. Each test file declares
 * its cases through {@link test}/{@link assertEqual}/{@link assertTrue}; a runner
 * (run-readiness-tests.ts) collects the registry and reports pass/fail.
 *
 * Strictly local. No execution, no submit, no wallet — paper/shadow only.
 */
...
export function assertTrue(cond: unknown, msg: string): void {
  if (cond !== true) {
    throw new Error(`assertTrue failed: ${msg} (got ${String(cond)})`);
  }
}

export function assertFalse(cond: unknown, msg: string): void {
  if (cond !== false) {
    throw new Error(`assertFalse failed: ${msg} (got ${String(cond)})`);
  }
}
...
export function assertIncludes(haystack: string, needle: string, msg: string): void {
  if (!haystack.includes(needle)) {
    throw new Error(`assertIncludes failed: ${msg} — expected to find "${needle}"`);
  }
}
