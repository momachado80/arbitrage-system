/**
 * Structural Late Exit Tighter Challenger Diagnostics — shadow_1000_structural_lateexit_tighter_v1.
 * Hipótese: mesma lógica de late exit que nonreversion, com thresholds apertados.
 * Mudança única: stagnantEdgeFloor 0.03→0.045, netEdgeProlongedFloor 0.02→0.03.
 */

import type { ShadowProfileState } from "./shadowSimulationStore";
import type { ClosedTradeAuditEntry } from "./shadowClosedTradeAudit";

const PROFILE_ID = "shadow_1000_structural_lateexit_tighter_v1";
...
export interface StructuralLateExitTighterDiagnosticsBlock {
  profileId: string;
...
export interface StructuralLateExitTighterComparisonBlock {
...
export function getStructuralLateExitTighterDiagnostics(
...
): StructuralLateExitTighterDiagnosticsBlock {
...
export function getStructuralLateExitTighterComparison(
  profiles: ShadowProfileState[],
  diagnostics: StructuralLateExitTighterDiagnosticsBlock,
  compareProfileId: string
): StructuralLateExitTighterComparisonBlock {
  ...
  return {
    ...
    sameUniverseNote: `Challenger: structural late exit TIGHTER (stagnant 4.5%, netEdge 3%). Baseline: ${compareProfileId}.`,
  };
}

export const STRUCTURAL_LATE_EXIT_TIGHTER_PROFILE_ID = PROFILE_ID;
